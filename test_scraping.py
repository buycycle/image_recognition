...
def save_result(result_file_path, row, response, response_text, vision_api_tokens, matches_df):
    ...
    with open(result_file_path, 'a') as file:
        ...
        file.write(f"Bike emplate: {row["template_id"]}  {row["template_name"]}\n")
...
def process_row(row):
    ...
    return pd.Series([match_tids, template_check, most_common_family_id, family_check])


# add result to df_test and save it
df_test[['match_ids', 'template_check', 'match_family_id', 'family_check']] = df_test.apply(process_row, axis=1) 
